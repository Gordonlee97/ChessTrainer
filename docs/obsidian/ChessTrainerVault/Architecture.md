...
| `src/engine/` | UCI protocol parsing, the Stockfish Worker transport, the `Engine` class that serializes searches, and a FEN-keyed eval cache (`evalCache.ts`) shared across transposed positions |
